...
std_msgs::Int32MultiArray for_st2_com;
...
void publish_ST2_com(){
    //~ for ( int i = 0; i < 6; i++){
        //~ for_st2_com.data.push_back(ST2_tx[i]);
        //~ ROS_INFO("publish in for %d %d", ST2_tx[i], for_st2_com.data[i]);
    //~ }
    //~ for( int i =0; i < 500; i ++){
		
    //~ forST2com.publish(for_st2_com);
//~ }
    //~ ST2_tx.clear(); 
    //~ for_st2_com.data.clear(); 
}
void publish_ST2(){
	ROS_INFO("to publish st2 com");
	publish_ST2_com();
    //~ for ( int i = 0; i < 6; i++){
